...
Employee* Manager::select_employee(Employee::KeyType type, const std::string& value) {
    for (int i = 0; i < employees.length; i++) {
        ...
        if (type == Employee::KeyType::ID) {
            if (to_string(employees.elem[i].id) == value) found = true;
        }
        else if (type == Employee::KeyType::NAME) {
            if (employees.elem[i].name == value) found = true;
        }
        else if (type == Employee::KeyType::ROLE) {
            if (employees.elem[i].role == value) found = true;
        }
...
void Manager::update_employee(int id) {
    ...
    cout << "请输入新职务: "; cin >> emp->role;
    cout << "请输入新电话: "; cin >> emp->phonenumber;
    cout << "请输入新住址: "; cin >> emp->address;
...
void Manager::show_all() {
    ...
    for (int i = 0; i < employees.length; i++) {
        // 打印每个员工的信息，中间用制表符 \t 隔开
        cout << employees.elem[i].id << "\t" 
             << employees.elem[i].name << "\t"
             << employees.elem[i].gender << "\t"
             << employees.elem[i].role << "\t"
             << employees.elem[i].phonenumber << "\t"
             << employees.elem[i].address << endl;
    }
}
...
bool Manager::compare(const Employee& a, const Employee& b, Employee::KeyType type) {
    if (type == Employee::KeyType::ID) return a.id <= b.id;
    if (type == Employee::KeyType::NAME) return a.name <= b.name;
    if (type == Employee::KeyType::ROLE) return a.role <= b.role;
...
int Manager::partition(int low, int high, Employee::KeyType type) {
    ...
    while (low < high) {
        // 从右往左找比基准小的
        while (low < high && !compare(employees.elem[high], pivot, type)) high--;
        employees.elem[low] = employees.elem[high];
        
        // 从左往右找比基准大的
        while (low < high && compare(employees.elem[low], pivot, type)) low++;
